...
const candidateSearch = {
  getAcademicTypes: (lang: string) => {
    const URL = `https://apr-mentioned-accompanied-katrina.trycloudflare.com/api/v3/academic-types?lang=${lang}`
  ...
  getCandidates: (addresses: any, categories: any, educations: number | undefined, gender: number | undefined, ageMin: number | null, ageMax: number | null, limit: number | null, page: number | null, lang: string) => {
    ...
    const URL = `https://apr-mentioned-accompanied-katrina.trycloudflare.com/api/v3/cv-filter/search?` +
      `${addresses.length !== 0
  ...
  postBookmarkCandidate: (accountId: string) => {
    const URL = `https://apr-mentioned-accompanied-katrina.trycloudflare.com/api/v3/candidate-bookmarks`
  ...
  getBookmarkCandidate: (page: number, limit: number, lang: string) => {
    const URL = `https://apr-mentioned-accompanied-katrina.trycloudflare.com/api/v3/candidate-bookmarks?lang=${lang}&page=${page}&limit=${limit}`
    return axiosClientRecruiter.get(URL, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('accessTokenRecruiter')}`,
      },
  ...
  postCountShowCandidate: (accountId: string) => {
    const URL = `https://apr-mentioned-accompanied-katrina.trycloudflare.com/api/v3/view-profiles`
    return axiosClientRecruiter.post(
      URL,
      { profileId: accountId },
      {
