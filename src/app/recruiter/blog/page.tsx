...
import ComunityRecruiter from "@/components/CommunityComponent/DetailCommunityRecruiter";
...
const page = (props: Props) => {
  ...
  const fetchData = async () => {
    ...
    const resUser = await communityApi.getCommunityByAccountRecruiter(
      "0",
      "3",
      "cm",
  ...
  const handleLikeCommunity = async (communicationId: number) => {
    try {
      const result = await communityApi.postCommunityLikeRecruiter(
        communicationId
      );
      if (result) {
        result.status === 201
            ...
            <div className="bg-white/50 flex rounded-lg overflow-hidden items-center p-4 gap-2 w-full">
              <div className="w-10 h-10 rounded-full overflow-hidden">
                <Image
                  ...
                  alt=""
                  className=""
                  src={profile?.companyInfomation?.logoPath ?? "/goapply.png"}
                />
              </div>
              <div
      ...
      {tabModal && (
        ...
        >
          ...
          >
            ...
            <ComunityCreatePostRecruiter setTab={handleUpload} />
          </div>
        </div>
      )}
      {tabModalDetail && (
        ...
        >
          ...
          >
            ...
            <ComunityRecruiter idCommunity={idCommunity} />
          </div>
        </div>
      )}
    </div>
