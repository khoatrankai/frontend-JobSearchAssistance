...
type IEditPostAddress = {
  setDataCompany: any;
  dataCompany: any;
  is_profile: boolean;
};

const ModifyRoleWebComponent: React.FC<any> = memo(
  (props: IEditPostAddress) => {
      ...
      if (dataRoles && !selectedRole) {
        setSelectedRole(
          dataRoles?.find(
            (dataRole: any) =>
              dataRole?.nameText ===
              dataCompany?.companyRoleInfomation?.nameText
          )
        );
      }
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField
                {...params}
                placeholder={
                  languageRedux === 1 ? "Chọn vai trò" : "Select role"
                }
                size="small"
              />
            )}
