/* eslint-disable react/jsx-key */
import React, { useEffect, useState } from "react";
import Image from "next/image";
import axiosClient from "@/configs/axiosClient";
import { useSelector } from "react-redux";
import { RootState } from "@/redux";
import { Input } from "antd";
import ToastCustom from "@/util/ToastCustom";

type Props = {
  dataInfo: any;
  handleUpdateApi: any;
  checkUpdate: boolean;
};
interface IData {
  statusCode: any;
  data: any;
}

const LanguageProfile = (props: Props) => {
  const { dataInfo, handleUpdateApi } = props;
  const [rsLanguage, setLanguage] = useState<boolean>(false);
  const languageRedux = useSelector(
    (state: RootState) => state.changeLaguage.language
  );
  const [dataRequest, setDataRequest] = useState<any>([]);
  const { hdError, hdSuccess } = ToastCustom();
  const [dataAdd, setDataAdd] = useState<any>([]);
  const [dataRemove, setDataRemove] = useState<any>({ ids: [] });
  const [dataLevel, setDataLevel] = useState<any>([]);

  useEffect(() => {
    setDataLevel([
      { id: 8, name: languageRedux === 1 ? "Sơ cấp" : "Basic" },
      { id: 9, name: languageRedux === 1 ? "Trung cấp" : "Intermediate" },
      { id: 10, name: languageRedux === 1 ? "Cao cấp" : "Advanced" },
      { id: 11, name: languageRedux === 1 ? "Thành thạo" : "Fluent" },
    ]);
  }, [languageRedux]);
  const handleCheckSame = (name: string, level: number) => {
    const dataArr = [...dataRequest, ...dataAdd];
    let keyMain = name + "|" + level;

    let seen = new Set();
    seen.add(keyMain);
    let duplicates = [];

    for (let item of dataArr) {
      let key = item.languageName + "|" + item.languageLevelId;
      if (seen.has(key)) {
        duplicates.push(item as never);
      }
    }
    if (duplicates.length > 1) {
      return true;
    }
    return false;
  };
  const handleUpdateRequest = (index: number, e: any, value?: number) => {
    switch (e.target.name) {
      case "languageName":
        setDataRequest(
          dataRequest.map((dt: any, i: number) => {
            if (i === index) {
              dt.languageName = e.target.value;
              dt.api = true;
              return dt;
            }
            return dt;
          })
        );
        break;
      case "languageLevelId":
        setDataRequest(
          dataRequest.map((dt: any, i: number) => {
            if (i === index) {
              dt.languageLevelId = value;
              dt.api = true;
              return dt;
            }
            return dt;
          })
        );
        break;
      default:
        setDataRequest(
          dataRequest.filter((dt: any, i: number) => {
            return i !== index;
          })
        );
        const data = dataRequest.filter((dt: any, i: number) => {
          return i === index;
        });
        if (dataRemove.ids.length > 0) {
          setDataRemove({ ids: [...dataRemove.ids, data[0].id] });
        } else {
          setDataRemove({ ids: [data[0].id] });
        }
    }
  };
  useEffect(() => {
    const newData = dataInfo?.profilesLanguages?.map((dt: any) => {
      return { ...dt, api: false };
    });
    setDataRequest(newData);
  }, [dataInfo?.profilesLanguages]);
  const handleCheckNameLevel = (id: number) => {
    const data = dataLevel.filter((dt: any) => {
      return dt.id === id;
    });
    return data[0]?.name;
  };
  const handleUpdateAdd = (index: number, e: any, value?: number) => {
    if (e.target.name === "languageName") {
      setDataAdd(
        dataAdd.map((dt: any, i: number) => {
          if (i === index) {
            dt.languageName = e.target.value;
            return dt;
          }
          return dt;
        })
      );
    } else {
      if (e.target.name === "languageLevelId") {
        setDataAdd(
          dataAdd.map((dt: any, i: number) => {
            if (i === index) {
              dt.languageLevelId = value;
              return dt;
            }
            return dt;
          })
        );
      } else {
        setDataAdd(
          dataAdd.filter((dt: any, i: number) => {
            return i !== index;
          })
        );
      }
    }
  };
  const handleAdd = () => {
    setDataAdd([...dataAdd, { languageName: "", languageLevelId: 8 }]);
  };
  const handleApiUpdate = async () => {
    let res = 200;
    for (let i of dataRequest) {
      const data = await fetchUpdata(i);
      if (!data) {
        res = 0;
      }
    }
    return res;
  };
  const fetchUpdata = async (dt: any) => {
    let check = true;
    if (dt.api) {
      const resUp = (await axiosClient.put(
        `https://lending-advantage-pale-xp.trycloudflare.com/api/v3/profile-languages/${dt.id}`,
        { languageName: dt.languageName, languageLevelId: dt.languageLevelId }
      )) as unknown as IData;
      if (resUp?.statusCode !== 200) {
        check = false;
      }
    }
    return check;
  };
  const handleApiCreate = async () => {
    let res = 201;
    for (let i of dataAdd) {
      const data = await fetchCreatedata(i);
    }
    return res;
  };
  const fetchCreatedata = async (dt: any) => {
    const resUp = (await axiosClient.post(
      "https://lending-advantage-pale-xp.trycloudflare.com/api/v3/profile-languages",
      dt,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
        },
      }
    )) as unknown as IData;
    if (resUp?.statusCode === 201 && resUp) {
      return true;
    }
    return false;
  };
  const handleUpdateData = () => {
    const fetchData = async () => {
      let resRemove = { statusCode: 200, data: [] };
      if (dataRemove.ids.length > 0) {
        resRemove = (await axiosClient.delete(
          "https://lending-advantage-pale-xp.trycloudflare.com/api/v3/profile-languages/remove",
          { data: dataRemove }
        )) as unknown as IData;
      }

      if (resRemove.statusCode === 200 && resRemove) {
        setDataRemove({ ids: [] });
        const resUp = await handleApiUpdate();
        if (resUp === 200) {
          const resCre = await handleApiCreate();
          if (resCre === 201) {
            hdSuccess("Cập nhật thông tin ngoại ngữ thành công");
            setDataAdd([]);
            handleUpdateApi();
            setLanguage(!rsLanguage);
          } else {
            hdError("Cập nhật thông tin ngoại ngữ không thành công");
          }
        }
      }
    };
    fetchData();
  };
  function checkDuplicates(arr: any) {
    let seen = new Set();
    let duplicates: any = [];

    for (let item of arr) {
      let key = item.languageName + "|" + item.languageLevelId;
      if (seen.has(key)) {
        duplicates.push(item as never);
      } else {
        seen.add(key);
      }
    }

    return duplicates;
  }
  const handlersLanguage = (name: any) => {
    switch (name) {
      case "update":
        setLanguage(!rsLanguage);
        break;
      case "save":
        const dataarr = [...dataRequest, ...dataAdd];
        if (checkDuplicates(dataarr).length === 0) handleUpdateData();
        break;
      case "close":
        setLanguage(!rsLanguage);
        break;
    }
  };
  return (
    <div
      className={` bg-white ${
        rsLanguage
          ? "shadow-[rgba(13,_38,_76,_0.19)_0px_9px_20px] "
          : "border-transparent"
      } p-4 rounded-xl mb-4 relative`}
    >
      <div className="flex justify-between flex-wrap mb-8">
        <div className="flex h-fit items-center">
          <div className="h-10 w-3 bg-blue-500 mr-4"></div>
          <h1 className="font-bold text-xl">
            {languageRedux === 1 ? "Ngoại ngữ" : "Language"}
          </h1>
        </div>

        {props.checkUpdate === false && (
          <div className="flex gap-2">
            {rsLanguage ? (
              <>
                <button
                  className="w-fit flex items-center hover:shadow-[inset_-12px_-8px_40px_#46464620] rounded-lg p-1 font-serif"
                  onClick={() => {
                    handlersLanguage("save");
                  }}
                >
                  <Image
                    className="w-4 mr-1"
                    src={"/icontick.svg"}
                    alt=""
                    width={200}
                    height={200}
                  />
                  <h2 className="text-sm font-bold">
                    {languageRedux === 1 ? "Lưu" : "Save"}
                  </h2>
                </button>
                <button
                  className="w-fit flex items-center hover:shadow-[inset_-12px_-8px_40px_#46464620] rounded-lg p-1 font-serif"
                  onClick={() => {
                    handlersLanguage("close");
                  }}
                >
                  <Image
                    className="w-4 mr-1"
                    src={"/iconclose.svg"}
                    alt=""
                    width={200}
                    height={200}
                  />
                  <h2 className="text-sm font-bold">
                    {languageRedux === 1 ? "Thoát" : "Exit"}
                  </h2>
                </button>
              </>
            ) : (
              <button
                className="w-fit flex items-center hover:shadow-[inset_-12px_-8px_40px_#46464620] rounded-lg p-1 font-serif"
                onClick={() => {
                  handlersLanguage("update");
                }}
              >
                <Image
                  className="w-4 mr-1"
                  src={"/iconwrite.svg"}
                  alt=""
                  width={200}
                  height={200}
                />
                <h2 className="text-sm font-bold">
                  {languageRedux === 1 ? "Sửa" : "Edit"}
                </h2>
              </button>
            )}
          </div>
        )}
      </div>
      <div>
        <ul className="flex flex-col gap-y-2">
          {dataRequest?.map((dt: any, index: number) => {
            return (
              <li
                key={index}
                className={`border-2  rounded-lg p-2 relative group ${
                  handleCheckSame(dt.languageName, dt.languageLevelId)
                    ? "border-red-500"
                    : " border-transparent focus-within:border-black/20 hover:border-black/20"
                }`}
              >
                <Input
                  className={`text-base w-full outline-none border-[1px] p-1 border-transparent border-dashed ${
                    rsLanguage
                      ? `hover:border-black/10 focus-within:border-black/10`
                      : ""
                  }`}
                  name="languageName"
                  onChange={(e) => {
                    handleUpdateRequest(index, e);
                  }}
                  value={dt?.languageName}
                  disabled={!rsLanguage}
                  placeholder={
                    languageRedux === 1 ? "Tên kỹ năng" : "Skill name"
                  }
                />

                <div
                  className={`text-base w-full mb-2 outline-none border-[1px] p-1 border-transparent border-dashed ${
                    rsLanguage
                      ? "hover:border-black/10 focus-within:border-black/10"
                      : ""
                  }`}
                >
                  <div className="flex items-center">
                    <h2 className="text-gray-400 mr-1 font-serif">
                      {languageRedux === 1 ? "Cấp độ:" : "Level:"}
                    </h2>
                    <h2 className="font-bold font-serif">
                      {handleCheckNameLevel(dt?.languageLevelId)}
                    </h2>
                  </div>
                  <div>
                    <ul className="w-full flex">
                      <li className="basis-1/4">
                        <button
                          onClick={(e) => {
                            handleUpdateRequest(index, e, 8);
                          }}
                          disabled={!rsLanguage}
                          name="languageLevelId"
                          className={`w-full h-4 border-2 transition-all  ${
                            dt.languageLevelId === 8
                              ? "bg-green-600"
                              : rsLanguage && "hover:bg-green-500"
                          }`}
                        ></button>
                      </li>
                      <li className="basis-1/4">
                        <button
                          onClick={(e) => {
                            handleUpdateRequest(index, e, 9);
                          }}
                          disabled={!rsLanguage}
                          name="languageLevelId"
                          className={`w-full h-4 border-2 transition-all  ${
                            dt.languageLevelId === 9
                              ? "bg-blue-600"
                              : rsLanguage && "hover:bg-blue-500"
                          }`}
                        ></button>
                      </li>
                      <li className="basis-1/4">
                        <button
                          onClick={(e) => {
                            handleUpdateRequest(index, e, 10);
                          }}
                          disabled={!rsLanguage}
                          name="languageLevelId"
                          className={`w-full h-4 border-2 transition-all  ${
                            dt.languageLevelId === 10
                              ? "bg-orange-600"
                              : rsLanguage && "hover:bg-orange-500"
                          }`}
                        ></button>
                      </li>
                      <li className="basis-1/4">
                        <button
                          onClick={(e) => {
                            handleUpdateRequest(index, e, 11);
                          }}
                          disabled={!rsLanguage}
                          name="languageLevelId"
                          className={`w-full h-4 border-2 transition-all  ${
                            dt.languageLevelId === 11
                              ? "bg-red-600"
                              : rsLanguage && "hover:bg-red-500"
                          }`}
                        ></button>
                      </li>
                    </ul>
                  </div>
                </div>
                {rsLanguage && (
                  <button
                    className="p-2 rounded-full absolute hidden -top-3 -right-3 bg-white border-2 group-hover:block"
                    name="deleteSkill"
                    onClick={(e) => {
                      handleUpdateRequest(index, e);
                    }}
                  >
                    <Image
                      className="w-4"
                      src={"/iconclose.svg"}
                      alt=""
                      width={200}
                      height={200}
                    />
                  </button>
                )}
              </li>
            );
          })}
          {dataAdd.map((dt: any, index: number) => {
            return (
              <li
                key={index}
                className={`border-2  rounded-lg p-2 relative group ${
                  handleCheckSame(dt.languageName, dt.languageLevelId)
                    ? "border-red-500"
                    : " border-transparent focus-within:border-black/20 hover:border-black/20"
                }`}
              >
                <Input
                  className={`text-base w-full outline-none border-[1px] p-1 border-transparent border-dashed ${
                    rsLanguage
                      ? "hover:border-black/10 focus-within:border-black/10"
                      : ""
                  }`}
                  name="languageName"
                  onChange={(e) => {
                    handleUpdateAdd(index, e);
                  }}
                  value={dt?.languageName}
                  disabled={!rsLanguage}
                  placeholder={
                    languageRedux === 1 ? "Tên kỹ năng" : "Skill name"
                  }
                />

                <div
                  className={`text-base w-full mb-2 outline-none border-[1px] p-1 border-transparent border-dashed ${
                    rsLanguage
                      ? "hover:border-black/10 focus-within:border-black/10"
                      : ""
                  }`}
                >
                  <div className="flex items-center">
                    <h2 className="text-gray-400 mr-1 font-serif">
                      {languageRedux === 1 ? "Cấp độ:" : "Level:"}
                    </h2>
                    <h2 className="font-bold font-serif">
                      {handleCheckNameLevel(dt?.languageLevelId)}
                    </h2>
                  </div>
                  <div>
                    <ul className="w-full flex">
                      <li className="basis-1/4">
                        <button
                          onClick={(e) => {
                            handleUpdateAdd(index, e, 8);
                          }}
                          disabled={!rsLanguage}
                          name="languageLevelId"
                          className={`w-full h-4 border-2 transition-all  ${
                            dt.languageLevelId === 8
                              ? "bg-green-600"
                              : rsLanguage && "hover:bg-green-500"
                          }`}
                        ></button>
                      </li>
                      <li className="basis-1/4">
                        <button
                          onClick={(e) => {
                            handleUpdateAdd(index, e, 9);
                          }}
                          disabled={!rsLanguage}
                          name="languageLevelId"
                          className={`w-full h-4 border-2 transition-all  ${
                            dt.languageLevelId === 9
                              ? "bg-blue-600"
                              : rsLanguage && "hover:bg-blue-500"
                          }`}
                        ></button>
                      </li>
                      <li className="basis-1/4">
                        <button
                          onClick={(e) => {
                            handleUpdateAdd(index, e, 10);
                          }}
                          disabled={!rsLanguage}
                          name="languageLevelId"
                          className={`w-full h-4 border-2 transition-all  ${
                            dt.languageLevelId === 10
                              ? "bg-orange-600"
                              : rsLanguage && "hover:bg-orange-500"
                          }`}
                        ></button>
                      </li>
                      <li className="basis-1/4">
                        <button
                          onClick={(e) => {
                            handleUpdateAdd(index, e, 11);
                          }}
                          disabled={!rsLanguage}
                          name="languageLevelId"
                          className={`w-full h-4 border-2 transition-all  ${
                            dt.languageLevelId === 11
                              ? "bg-red-600"
                              : rsLanguage && "hover:bg-red-500"
                          }`}
                        ></button>
                      </li>
                    </ul>
                  </div>
                </div>
                {rsLanguage && (
                  <button
                    className="p-2 rounded-full absolute hidden -top-3 -right-3 bg-white border-2 group-hover:block"
                    onClick={(e) => {
                      handleUpdateAdd(index, e);
                    }}
                  >
                    <Image
                      className="w-4"
                      src={"/iconclose.svg"}
                      alt=""
                      width={200}
                      height={200}
                    />
                  </button>
                )}
              </li>
            );
          })}
        </ul>
      </div>
      <div className="flex justify-center mt-4">
        {rsLanguage && (
          <button
            className="flex items-center transition-all duration-300 hover:bg-black/25 p-1 rounded-2xl"
            onClick={handleAdd}
          >
            <Image
              className="w-6 mr-1 p-1 bg-black/20 rounded-full"
              src={"/iconadd.svg"}
              alt=""
              width={200}
              height={200}
            />
            <h2 className="font-bold">
              {languageRedux === 1 ? "Thêm ngoại ngữ" : "Add language"}
            </h2>
          </button>
        )}
      </div>
    </div>
  );
};

export default LanguageProfile;
