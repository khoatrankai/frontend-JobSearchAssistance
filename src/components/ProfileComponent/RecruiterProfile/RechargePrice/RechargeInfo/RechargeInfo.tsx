...
const RechargeInfo = (props: Props) => {
  ...
  const dataPay = ["THẺ ATM - IBAKING", "THẺ QUỐC TẾ", "VNPAY", "MOBILE MONEY"];
  const [checkValuePay, setCheckValuePay] = useState<any>(-1);
  const handlePay = async () => {
    if (profile?.companyInfomation?.isActive) {
      if (checkValuePay === 3) {
        if (checkValue === dataPrice.length) {
          const dataRes: any = await apiPayment.createPayMomo(valueOther);
          if (dataRes?.moMoResponse) {
            //   setCookieCustom("selectProfileRecruiter", 2);
            pushRouter(dataRes?.moMoResponse?.payUrl);
          } else {
            hdError("Lỗi không thể xử lý");
          }
        } else {
          const dataRes: any = await apiPayment.createPayMomo(
        ...
        if (checkValue === dataPrice.length) {
          const dataRes: any = await apiPayment.createPay(valueOther);
          if (dataRes && dataRes?.statusCode === 200) {
            //   setCookieCustom("selectProfileRecruiter", 2);
            pushRouter(dataRes.data.url);
          } else {
            hdError("Lỗi không thể xử lý");
          }
        } else {
          const dataRes: any = await apiPayment.createPay(
            dataPrice[checkValue]
          );
          if (dataRes && dataRes?.statusCode === 200) {
            pushRouter(dataRes.data.url);
      ...
      hdError("Vui lòng chờ xác thức trước khi sử dụng chức năng này");
    }
  };
  return (
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-4 w-fit justify-center">
            ...
            >
              ...
              <p className="font-bold text-blue-800">THẺ QUỐC TẾ</p>
            </button>
            <button
              className={`w-44 h-28 relative shadow-2xl shadow-blue-500/20 rounded-md border-[1px] transition-all duration-500  group  flex items-end justify-center ${
                checkValuePay === 2
            ...
            >
              <div className="absolute inset-0 flex justify-center items-center">
                <p
                  className={`text-2xl  font-extrabold transition-all duration-500 group ${
                    checkValuePay === 2
                      ? "text-red-600"
              ...
              <p className="font-bold text-blue-800">VNPAY</p>
            </button>
            <button
              className={`w-44 h-28 relative shadow-2xl shadow-blue-500/20 rounded-md border-[1px] transition-all duration-500  group  flex items-end justify-center ${
                checkValuePay === 3
