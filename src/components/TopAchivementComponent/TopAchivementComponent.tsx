...
const TopAchivementComponent = (props: Props) => {
  ...
  //   }, []);
  useEffect(() => {}, []);
  const handleAddClassBubble = (data: any) => {
  ...
  return (
    <div className="gradient-bg-mid w-full relative flex flex-col items-center py-8 h-[440px] ">
      ...
      <div className="max-w-6xl w-full h-full flex flex-col justify-center items-start gap-y-4 relative ">
        <p className="capitalize font-bold text-4xl text-black/75 w-2/3 relative z-10">
          Những Nhà Tuyển dụng nổi bật nhất tháng cùng{" "}
          <span className="text-blue-700">JOB IT</span>
        </p>
        <p className=" font-medium text-lg text-black/50 w-2/3 relative z-10">
      ...
      <div className="absolute inset-0 overflow-hidden">
        {dataCompany.map((dt: any) => {
          return handleAddClassBubble(dt);
        })}
      </div>
    </div>
  );
