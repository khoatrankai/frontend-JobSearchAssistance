...
import "./SearchJobMapComponent.scss";
...
const SearchJobMapComponent: React.FC<Props> = (props) => {
  ...
  const handleSearch = async () => {
    const dataSearch = await mapApi.getSearchMapLocation(
      dataRequest,
      dataLocation,
      1
    );
    if (dataSearch) {
      setListJob(dataSearch.data?.posts);
      setPage(dataSearch.data?.currentPage);
    }
  };
  useEffect(() => {
  ...
  const loadMore = async () => {
    ...
    if (dataSearch) {
      setListJob([...listJob, ...dataSearch.data?.posts]);
      setPage(dataSearch.data?.currentPage);
      setLoading(false);
    }
  };
      ...
      <div className="flex justify-center py-12 bg-blue-50 px-4">
        <div className="w-full max-w-6xl relative">
          <h1 className="font-bold text-2xl mb-3">{`${
            listJob?.length ? listJob?.length : 0
          } kết quả tìm kiếm`}</h1>
...
export default SearchJobMapComponent;
