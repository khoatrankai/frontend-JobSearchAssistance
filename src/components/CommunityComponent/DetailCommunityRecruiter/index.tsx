...
const ComunityRecruiter = () => {
  ...
  const handleDeleteCmt = async (postId: any, cmtId: any) => {
    try {
      const result = await communityApi.deleteCommentRecruiter(postId, cmtId);
  ...
  const handleSaveCommunity = async (communicationId: number) => {
    ...
    try {
      const result = await communityApi.postCommunityBookmarkedRecruiter(
        communicationId
      );
      if (result) {
  ...
  const handleCommentCommunity = async () => {
    ...
    try {
      const result = await communityApi.postCommunityCommentRecruiter(form);
      if (result) {
        setCmt(!cmt);
        setCmtContent("");
      }
  ...
  const handleMoveToList = () => {
    ...
    fromHistory === "31"
      ...
      : window.open(
          detail?.type === 1 ? "/recruiter/blog" : "/recruiter/blog",
          "_parent"
        );
  };
...
export default ComunityRecruiter;
