    ...
    <>
      {profile && profile.profilesCvs ? (
        <div className="container">
          <FormControlLabel
            control={
              <Radio checked={isCheckAllCv} onChange={handleRadioChange} />
            }
            label={
              <span
          ...
          {isCheckAllCv && (
            <>
              {profile?.profilesCvs?.length > 0 ? (
                profile.profilesCvs.map((cv: any, index: number) => (
                  <div
                    key={index}
                    className={
                      idSelectFromAllCv !== cv.id
                        ? "item-all-cv cursor-pointer"
                        : "item-all-cv select-item cursor-auto"
                    }
                    onClick={() => {
